...
import com.laughoutloud.ClassProgressDialog
import com.laughoutloud.ClassSharedPreferences
import com.laughoutloud.ClassUtilities
...
class ActivityCropImage : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(prefs.getQImgUrl() !=""){
            mSourceUri = Uri.fromFile(File(prefs.getQImgUrl()))
            prefs.setScanCheck("")//set empty on loading
        }else{
            super.onBackPressed()
        }
...
//        mSourceUri = Uri.fromFile(File("/sdcard/sample.jpg"))
    ...
    private val mSaveCallback = object : SaveCallback {
        override fun onSuccess(uri: Uri?) {
            ...
            prefs.setQImgUrl(outputFilePath)
            backPressed()
            finish()
        }
